...
fn resolve(manifest: &Path, target: &str, flavour: Flavour, version: &str) -> (PathBuf, String) {
    ...
    // than by the library version. Two releases of the same fdk-aac carry different archives —
    // an option changed, a patch applied, a flag corrected — and a cache directory named after the
    ...
    let stale = |why: String| {
        println!(
            "cargo:warning=cannot ask {repo} which release is latest ({why}) — falling back to \
             the newest cached fdk-aac archive, which may be out of date"
        );
        None
    };
...
/// `$CARGO_HOME/fdk-aac-prebuilt/<release tag>/`, one directory per release, so the download
