// Find the prebuilt fdk-aac for this target and emit the link flags. What this build script
// does *not* do is the point of the crate: no cmake, no C++ compiler, no autotools, no
// vendored source tree, no `cc`. Compare `fdk-aac-sys`, which compiles ~170 C++ files here
// on every clean build.
//
// Three places an archive can come from, tried in this order:
//
//   1. `FDK_AAC_PREBUILT_DIR` — a prefix you built or unpacked yourself. Used as-is.
...
// killed build, an `.a` overwritten by hand — and each of those otherwise surfaces as a page
// of undefined symbols rather than one sentence naming the file.
//
// The checksum that constrains somebody *other than us* is the third-party one: fdk-aac.env
...
// has to be updated in here. The asset name carries the fdk-aac version, so a release of a
// *different* version cannot satisfy the URL — it 404s naming the version rather than
// quietly returning the wrong library.
use std::path::{Path, PathBuf};
use std::process::Command;

fn main() {
    println!("cargo:rerun-if-env-changed=FDK_AAC_PREBUILT_DIR");
    ...
    assert!(
        ...
        "no {archive} in {} (from {provenance})\n\nFDK_AAC_PREBUILT_DIR must name a \
    ...
    link_cxx_runtime(&target, manifest_text.as_deref(), &provenance);

    // For a consumer compiling its own C against the same headers, via the
    // `DEP_FDK_AAC_INCLUDE` that `links = "fdk-aac"` exposes.
    ...
    // build of every consumer is noise that teaches people to ignore warnings. Visible under
    // `cargo build -vv`, which is where the README says to look.
    println!("cargo:info=fdk-aac {version} linked statically from {provenance} ({target})");
    if let Some(text) = manifest_text.as_deref() {
        // The CPU floor and the C++ runtime finding, echoed into the log of the build that
        // produced the binary — which is where anyone debugging an illegal instruction on an
        // old machine, or a missing libstdc++ in a slim image, will look first.
        for line in
            text.lines().filter(|l| l.starts_with("cpu_floor") || l.starts_with("cxx_runtime"))
        {
            println!("cargo:info=fdk-aac {line}");
        }
    }
    if provenance == "FDK_AAC_PREBUILT_DIR" {
        // This one earns a warning: the archive came from outside the repo, so nothing
        // checked its version, its CPU floor, or what it links against.
        println!(
            "cargo:warning=fdk-aac from FDK_AAC_PREBUILT_DIR ({}) — unverified",
            prefix.display()
        );
    }
}

/// Emit the C++ runtime link flag, but only if the archive actually needs one.
///
/// Upstream `fdk-aac-sys` emits `-lstdc++` on Linux and `-lc++` on macOS unconditionally, so
/// every consumer of it acquires a C++ runtime dependency whether or not one is required.
/// It usually is not: cmake compiles fdk-aac with `-fno-exceptions -fno-rtti` and sets
/// `LINKER_LANGUAGE C`, and the source uses no `new`, no `delete`, no STL and no virtual
/// functions — it is C written in C++ files.
///
/// "Usually" is not good enough to hard-code either way, so `build.sh` measures it — it
/// lists the archive's undefined symbols and writes `cxx_runtime none` or
/// `cxx_runtime required: …` into the MANIFEST — and this reads the answer. The upshot for a
/// consumer is a binary with one fewer runtime dependency, which matters exactly where slim
/// images and static linking do.
///
/// An archive with no MANIFEST (only `FDK_AAC_PREBUILT_DIR` may lack one) gets the
/// conservative answer, because an unnecessary `-lstdc++` merely adds a dependency while a
/// missing one is a page of undefined symbols.
fn link_cxx_runtime(target: &str, manifest_text: Option<&str>, provenance: &str) {
    let measured = manifest_text
        .and_then(|text| text.lines().find_map(|line| line.strip_prefix("cxx_runtime ")));

    let needed = match measured {
        Some("none") => {
            println!("cargo:info=fdk-aac needs no C++ runtime (measured at build time)");
            false
        }
        Some(other) => {
            println!("cargo:info=fdk-aac needs a C++ runtime — {other}");
            true
        }
        None => {
            // Windows archives are the ones that legitimately arrive without a measurement:
            // `nm` does not exist there, so build.sh cannot ask. MSVC needs no flag from us
            // in any case — the C++ runtime is part of the CRT the linker already pulls in.
            if !target.contains("windows-msvc") {
                println!(
                    "cargo:warning=no cxx_runtime line in the MANIFEST (from {provenance}) — \
                     linking the C++ runtime to be safe"
                );
            }
            true
        }
    ...
    // MSVC: nothing. The CRT covers it.
}

/// Hash the archive on disk and require it to be what its own MANIFEST says was built.
/// Returns the MANIFEST text, since every later step wants to read a line out of it.
///
...
/// Hashing two megabytes costs a couple of milliseconds and turns a corrupt file into a
...
fn verify_library(prefix: &Path, library: &Path, provenance: &str) -> Option<String> {
    let text = match std::fs::read_to_string(prefix.join("MANIFEST")) {
        ...
        Err(_) if provenance == "FDK_AAC_PREBUILT_DIR" => return None,
        Err(e) => panic!(
    ...
    println!("cargo:info=fdk-aac sha256(library) {actual} matches the MANIFEST");
    Some(text)
}
...
fn fetch(manifest: &Path, name: &str, version: &str, cached: &Path) -> PathBuf {
    assert!(
        ...
        "fdk-aac for {name} is not cached and cargo is offline. Run ./build.sh {name} && \
         ./sync-prebuilt.sh, or set FDK_AAC_PREBUILT_DIR to a prefix containing lib/."
    );
    ...
    // requests would give a mismatch below rather than a wrong library — which is the right
    // way round for a race this unlikely.
    if !curl(&format!("{base}/SHA256SUMS"), &sums) {
        panic!(
            "cannot download {base}/SHA256SUMS\n\nEvery release publishes one beside the \
             ...
             FDK_AAC_PREBUILT_DIR to a prefix you built yourself."
        );
    }
...
/// release. It replaces relying on `tar` to notice: gzip's CRC does catch corruption, but it
/// reports it as "unexpected end of file" from a program the user did not know was running,
/// which is a worse sentence than this one.
...
/// `$CARGO_HOME/fdk-aac-prebuilt/`, so the download happens once per machine rather than
/// once per project — and so the many Docker builds that already cache `~/.cargo` get it for
    ...
    match target {
        ...
        "x86_64-apple-darwin" => panic!(
            "no prebuilt fdk-aac for Intel macOS: the macOS artifact is arm64, tuned for \
             apple-m1. Set FDK_AAC_PREBUILT_DIR to a prefix holding your own libfdk-aac.a, \
             or add the target to build.sh."
        ),
        other => panic!(
            "no prebuilt fdk-aac for {other}. Supported: aarch64-apple-darwin, \
             x86_64-unknown-linux-{{gnu,musl}}, aarch64-unknown-linux-{{gnu,musl}}, \
             x86_64-pc-windows-msvc. Set FDK_AAC_PREBUILT_DIR to a prefix holding your own \
             libfdk-aac.a for anything else."
        ),
    }
}

/// Read one setting out of `fdk-aac.env`, which is where the shell build keeps the same
/// values — parsed rather than duplicated, so the two halves of this repository cannot
/// disagree about which fdk-aac this is or where its archives live.
fn fdk_env(manifest: &Path, key: &str) -> String {
    let env_file = manifest.join("../../fdk-aac.env");
    ...
    text.lines()
        ...
        .unwrap_or_else(|| panic!("no {key} in fdk-aac.env"))
        .trim()
        .to_string()
}
...
///
/// Not a `#[cfg(test)] mod tests`, and that is the whole point of this comment. Cargo builds
/// a build script as a *binary it runs*, never as a test target — so a `#[test]` in this
/// file is compiled by nothing and run by nothing, and `cargo test` reports it as zero
/// tests passing while looking exactly like success. Fifty lines of hand-written hash
/// guarded by a test that does not exist is worse than fifty lines with no test at all,
/// because the second kind gets read carefully.
///
/// So it runs unconditionally, before the first hash that decides anything. Three short
/// vectors plus one megabyte of `'a'` — the long one is what exercises multi-block padding,
/// the part most likely to be wrong and least likely to show up on short inputs. The whole
/// check costs about a millisecond, once per build of this crate.
    ...
    for (input, expected) in [
        ...
        assert_eq!(
            ...
            "this build script's SHA-256 is wrong on a published test vector ({} bytes of \
             input). Every archive integrity check in this file is meaningless until that \
             is fixed.",
            input.len(),
        );
    }
